                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        if (data.is_success == true) {
                            ...
                            $('#buyButton').text('{{__('Pay Now')}} {{(env('CURRENCY_SYMBOL') ? env('CURRENCY_SYMBOL') : '$')}}'+data.price);
                            preciore = document.getElementById("buyButtons").value-data.price;
                            preciore = Number(preciore.toFixed(2));
                            $('#valtrpre').val(preciore);
                            $('#buyButtons').val(data.price);
                            $('#valcp').val(coupon);
                    ...
                    @endif
                        ...
                        <div id="paymentwall-payment" class="tabs-card {{ (isset($admin_payments_details['is_paymentwall_enabled']) && $admin_payments_details['is_paymentwall_enabled']=='on') ? "active" : "" }}">
                                ...
                                <div class="border p-3 mb-3 rounded payment-box">
                                    ...
                                    <div class="form-group mt-3 mr-3">
                                    <div class="text-sm-right">
                                    <button class="btn btn-primary btn-sm" id="buyButton">{{__('Pay Now')}} {{(env('CURRENCY_SYMBOL') ? env('CURRENCY_SYMBOL') : '$')}}{{$plan->price}}</button>
                                    </div>
                                    </div>
                                    <input type="hidden" id="buyButtons" value="{{$plan->price}}">
                                    <input type="hidden" id="valcp" value="">
                                    <input type="hidden" id="valtrpre" value="">
                        ...
                        <script>
                            ...
                            producto = 'Plan {{$plan->name}}';
                            precio = Math.round(document.getElementById("buyButtons").value*100);
                            idplan = {{$plan->id}};
                            ncup = document.getElementById("valcp").value;
                            dscup = document.getElementById("valtrpre").value;
                            cju = {{Auth::user()->id}};
                            nomu = '{{Auth::user()->name}}';
                            tipag = '{{__('PaymentWall')}}';
                            Culqi.publicKey = '{{ $admin_payments_details['paymentwall_public_key']  }}';
                            Culqi.settings({
                            title: '{{(\App\Models\Utility::getValByName('title_text')) ? \App\Models\Utility::getValByName('title_text') : env('APP_NAME', 'Llabook Shop')}}',
                            currency: 'PEN',
                        ...
                        function culqi() {
                          if (Culqi.token) {
                              ...
                              $.ajax({
                                  ...
                                  swal({title:'¡Gracias!', text: 'Se afilió con éxito al '+producto, icon: 'success', button: 'Ok',}).then((value) => {window.location='https://llabook.shop/plans';});
                              }).fail(function(resp){
                                  swal({title:'¡Error!', text: 'Hubo un error, por favor inténtelo de nuevo.', icon: 'error', button: 'Ok',});
                              });
                        ...
                        Culqi.options({
                            ...
                            installments: false,
                            style: {
                              logo: 'https://llabook.shop/storage/uploads/plan/plan_1646850162.png',
                              maincolor: '#051c4b',
                              buttontext: '#ffffff',
                              maintext: '#000000',
                              desctext: '#0097f8'
