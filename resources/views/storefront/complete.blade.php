...
<script>
    ...
    fbq('track', 'Purchase',{value: '{{$order->price}}', currency: '{{$order->price_currency}}'});
</script>


<script>
        var pixels = '{{$store_settings->tiktokpixel_code}}';
        const array_PixelsTikTok = pixels.split(',');
        //part1
        !function (w, d, t) {
        ...
        if (array_PixelsTikTok.length > 0) {
            console.log("vario varios");        
            array_PixelsTikTok.forEach(element => {
                //fbq('init', element);
                ttq.load(element);        
            });         
        } else {
        ...
        ttq.page();
        ttq.track('CompletePayment',{"content_type" :"product","content_id" :"401","value":'{{$order->price}}',"currency":'{{$order->price_currency}}'})       
</script>
