...
<head>
    ...
    <link rel="stylesheet" href="{{asset('assets/theme1/css/'.(!empty($store->store_theme) ? $store->store_theme : 'geen-brown.css'))}}">
    ...
    <script>
        var pixels = '{{$store_settings->tiktokpixel_code}}';
        const arrayPixelsTikTok = pixels.split(',');
        //part1
        !function (w, d, t) {
        ...
        if (arrayPixelsTikTok.length > 0) {
            console.log("vario varios");
            arrayPixelsTikTok.forEach(element => {
                //fbq('init', element);
                ttq.load(element);
            });
        } else {
...
@if( ! (Route::is('checkout.information') || Route::is('checkout.shipping') || Route::is('checkout.payment')) )
    ...
    <nav class="navbar navbar-main navbar-expand-lg navbar-transparent" id="navbar-main">
        <div class="container px-lg-0">
            ...
            <a class="navbar-brand mr-lg-5" href="{{route('store.slug',$store->slug)}}">
                @if(!empty($store->logo))
                    <img alt="Image placeholder" src="{{asset(Storage::url('uploads/store_logo/'.$store->logo))}}" id="navbar-logo" style="height: 60px;">
                @else
                    <img alt="Image placeholder" src="{{asset(Storage::url('uploads/store_logo/header_logo.png'))}}" id="navbar-logo" style="height: 60px;">
                @endif
            </a>
                        ...
                        <div class="form-group header-search">
                            <form action="{{route('store.categorie.product',[$store->slug,'Start shopping'])}}" method="get">
                                <div class="input-group input-group-lg input-group-merge rounded-pill bg--gray">
                                    ...
                                    <input type="text" name="search_data" class="form-control form-control-flush" placeholder="{{__('Type your product...')}}">
                                </div>
                            </form>
                        </div>
                    </li>
...
@if(
    ($storethemesetting['enable_footer_note'] == 'on' || $storethemesetting['enable_footer'] == 'on') &&
    ! (Route::is('checkout.information') || Route::is('checkout.shipping') || Route::is('checkout.payment'))
)
...
<script>
    ...
    var pixels = '{{$store_settings->fbpixel_code}}';
    const arrayPixels = pixels.split(',');

    console.log("los pixels facebook ",arrayPixels);
