...
<!--                         <div class="header__timer timer">
                        <p class="timer__remain">{{__('La promoción finalizará en:')}}</p>
                        <div class="timer-wrap">
                            <div class="timer-block"><span class="t-hour num"></span> <span class="name"> horas</span></div>
                            <div class="timer-block"><span class="t-min num"></span> <span class="name"> minutos</span></div>
                            <div class="timer-block"><span class="t-sec num"></span> <span class="name"> segundos</span></div>
                        </div>
                        </div> -->
                    </div>
                    <div class="cart-buttons">
                        <a href="#" style="width: 100%;" class="btn btn-primary rounded-pill btn-icon shadow hover-shadow-lg hover-translate-y-n3 add_to_cart" data-id="{{$products->id}}">
                            <span style="font-size: 21px;" class="btn-inner--text">{{__('Add to cart')}}</span>
            ...
            <div class="row row-grid">
                <div class="col-lg-12">
                    ...
                    <h5 class="mb-4" style="text-align: center;font-weight: 700;">{{__('SPECIFICATION')}}</h5>
                    <p class="text-sm mb-0 product-detail">{!! $products->specification !!}</p>
                    @endif
                    <br>
                    @if(!empty($products->detail))
                    <h5 class="mb-4" style="text-align: center;font-weight: 700;">{{__('DETAILS')}}</h5>
                    <p class="text-sm mb-0 product-detail">{!! $products->detail !!}</p>
                    @endif
               <br> <div class="customer-product-review">
...
<!--     <section class="top-product">
    ...
    <div class="cjcart">
    <div style="margin: 0px" class="cart-buttons">
    <a href="#" style="width: 100%;border-radius: 0px !important;" class="btn btn-primary rounded-pill btn-icon shadow hover-shadow-lg hover-translate-y-n3 add_to_cart" data-id="{{$products->id}}">
    <span style="font-size: 21px;" class="btn-inner--text">{{__('Add to cart')}}</span>
    <span class="btn-inner--icon">
    <i class="fas fa-shopping-basket"></i>
    </span>
...
@push('script-page')
    ...
    <script>
        var pixels = '{{$store_setting->fbpixel_code}}';
        const arrayPixelsFace = pixels.split(',');
        ...
        if (arrayPixelsFace.length > 0) {        
            arrayPixelsFace.forEach(element => {
                fbq('init', element);        
            });         
        } else {
            fbq('init', pixels);           
     ...
     <!-- TIKTOK PIXEL -->                                                   
    <script>
        var pixels = '{{$store_setting->tiktokpixel_code}}';
        const arrayPixels1 = pixels.split(',');
        //part1
        !function (w, d, t) {
        ...
        if (arrayPixels1.length > 0) {        
        arrayPixels1.forEach(element => {
            //fbq('init', element);
            ttq.load(element);        
        });         
        } else {
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (response.status == "Success") {
                        ...
                        var textnp = new String("{{$products->name}}");
                        var textnp = textnp.charAt(0).toUpperCase() + textnp.toLowerCase().slice(1);
                        var textadd = new String("{{ __('added to cart successfully!') }}");
                        textadd = textadd.toLowerCase();
                        window.location='{{route('store.cart',$store->slug)}}';
                        // Swal.fire({
                        //     text: textnp+" "+textadd,
                        //     icon: 'success',
                        //     showCancelButton: true,
                        //     confirmButtonColor: '#0f5ef8',
                        //     cancelButtonColor: '#082865',
                        //     cancelButtonText: '{{ __('Keep Buying') }}',
